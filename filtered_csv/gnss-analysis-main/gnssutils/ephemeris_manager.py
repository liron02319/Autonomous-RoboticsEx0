...
class EphemerisManager():
    ...
    def get_ephemeris(self, timestamp, satellites):
        ...
        # Option 1: Check actual column name
        if 'index' in data.columns:  # Check if 'index' column exists
            data = data.drop(columns=['index'], axis=1)
        else:
            data = data.iloc[:, 1:]  # Select all columns except the first one (presumed index)
